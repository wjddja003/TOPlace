...
public class QaService {
	public QaPageData adminSelectAll(int reqPage) {
		...
		ArrayList<QaComment> list = new QaDao().adminSelectAll(conn, start, end);
		String pageNavi = "";
		int pageNaviSize = 5;
		int pageNo = ((reqPage-1)/pageNaviSize)*pageNaviSize+1;
		if(pageNo != 1) {
			pageNavi += "<a class='btn' href='/adminQaPage?reqPage="+(pageNo-1)+"'>이전</a>";
		}
		int i = 1;
		while(!(i++>pageNaviSize || pageNo>totalPage)) {
			if(reqPage == pageNo) {
				...
				pageNavi += "<a class='btn' href='/adminQaPage?reqPage="+pageNo+"'>"+pageNo+"</a>";
			}
			pageNo++;
		}
	if(pageNo <= totalPage) {
		pageNavi +="<a class='btn' href='/adminQaPage?reqPage="+pageNo+"'>다음</a>";
	}
	...
	public int commentInsertQa(QaComment qc) {
		Connection conn = JDBCTemplate.getConnection();
		int result = new QaDao().commentInsertQa(conn, qc);
		if(result > 0) {
	...
	public QaPageData qaSelectList(int reqPageQna, int S_no) {
		...
		ArrayList<QaComment> list = new QaDao().selectList(conn, start, end,S_no);
		String pageNavi = "";
		int pageNaviSize = 5;
