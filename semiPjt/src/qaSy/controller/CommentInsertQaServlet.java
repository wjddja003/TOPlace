...
/**
 * Servlet implementation class CommentInsertQaServlet
 */
@WebServlet(name = "CommentInsertQa", urlPatterns = { "/commentInsertQa" })
public class CommentInsertQaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CommentInsertQaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int commentRef = Integer.parseInt(request.getParameter("qaNo"));
		System.out.println(commentRef);
		int ref = Integer.parseInt(request.getParameter("S_no"));
		System.out.println(ref);
		QaComment qc = new QaComment(0, qaCommentWriter, qaCommentContent, ref, null, commentRef);
		int result = new QaService().commentInsertQa(qc);
		if(result>0) {
